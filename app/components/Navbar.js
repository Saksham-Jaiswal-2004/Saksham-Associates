  ...
  return (
    <div>
      <nav className='flex justify-between items-center py-2 px-4 fixed top-0 w-full z-[101]' id='navbar'>
        ...
        <div className='flex md:gap-8 gap-3 items-center justify-center z-[103]'>
          <Link href="#Footer" id='helloBtn' className='md:text-xl text-xs social border rounded-2xl md:px-3 px-2 py-1 hover:border-[#D88E6C]'>Say Hello</Link>
      ...
      <div ref={divRef} id='collapse' className={`fixed py-4 px-8 z-[102] top-0 left-0 w-fit h-full backdrop-blur-md bg-[#33333380] text-white flex flex-col items-center justify-start space-y-8 text-2xl transition-transform duration-300 z-40"}`}  style={{transform: isOpen ? `translateX(${screenWidth - divWidth - 4}px)` : `translateX(${screenWidth}px)`,}}>
        ...
        <ul className='flex flex-col justify-center mt-14 text-lg items-start gap-4'>
          ...
          <li><Link href="/signup" onClick={closeMenu}>Sign Up</Link></li>
